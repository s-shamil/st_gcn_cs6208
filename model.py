...
def get_NTU_NormalizedAdj():
    # 1-indexed edges
    edges = [
        (21,3), (3,4), # Head
        (21,9), (9,10), (10,11), (11,12), (12,24), (12,25), # Right arm
        (21,5), (5,6), (6,7), (7,8), (8,22), (8,23), # Left arm
        (21,2), (2,1), # Trunk
    ...
    # A = torch.eye(25,dtype=torch.float)

    # For distance partitioning, return without self loop
    A = torch.zeros(25,25,dtype=torch.float)
...
class ST_GCN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.A_Norm = get_NTU_NormalizedAdj() # Adjacency matrix for NTU RGB+D dataset
...
if __name__=="__main__":
    # test model code
    mdl = ST_GCN(60).to(device)

    batch_size = 32
    num_channels = 3 # Input (x,y,z) 3d coord
    num_frames = 300 # Data either repeated or trimmed to adjust
    num_joints = 25 # For NTU RGB+D, 25 joints
    num_persons = 2 # Max 2 persons in the frame
